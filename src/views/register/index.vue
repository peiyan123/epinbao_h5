<template>
  <div class="px-80px h-100vh bg-default">
    <div class="h-38px py-60px">
      <div class="i-epinbao-left-arrow w-57px cursor-pointer" @click="handleLogin"></div>
    </div>
    <div class="font-size-88px mt-20px">个人注册-易聘宝</div>
...
<script setup>
  ...
  // api
  import { sendRegisterSms, smsRegister } from '@/api/api'
  // hooks
  ...
  async function onSubmit(values) {
    loading.value = true
    try {
      const res = await smsRegister(values)
      if (res.code === 200) {
        Toast(res.message)
        setTimeout(() => {
          router.push('/login')
